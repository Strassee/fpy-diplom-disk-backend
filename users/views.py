...
from storages.functions import mk_storage, replace_delete_user_storage
from users.permissions import IsAdmin, IsAdminOrOwner, IsNotOwnerDeleteOrReadPatchOnly, IsNotFirstAdminOrReadOnly
from users.serializers import LoginRequestSerializer, UserRegSerializer, UsersGetSerializer, UserGetSerializer, UserUpdateByAdminSerializer, UserUpdateByUserSerializer
...
def user_api(request, id):
  ...
  if request.method == 'GET':
    try:
      ...
      return Response(json.dumps({"errors": {"user": ["User not found"]}}), status=404)
...
def user_reg(request):
  ...
  if serializer.is_valid():
    user_new = User.objects.create_user(username=request.data['username'], first_name=request.data['first_name'], email=request.data['email'], password=request.data['password'])
    ...
    if authenticated_user is not None:
        ...
        return Response({'status reg': True, 'status login': True, 'user' : user_new.id, 'storage': authenticated_user.storage.id, 'admin' : authenticated_user.is_superuser}, status=201)
    else:
        return Response(json.dumps({"errors": {"username": ["Invalid credentials"]}}), status=403)
  else:
      return Response(json.dumps({"errors": serializer.errors}), status=400)
...
class UsersLogin(APIView):
  ...
  def get(self, request, format = None):
    ...
    if(path_arr[-1] == 'logout'):
      logout(request)
      return Response({'logout status': 'Success'}, status=200)
    return Response(json.dumps({"errors": {"path": ["Not found"]}}), status=404)
  ...
  def post(self, request):
    ...
    if(path_arr[-1] == 'login'):
      ...
      if serializer.is_valid():
        ...
        if authenticated_user is not None:
          login(request, authenticated_user)
          return Response({'status login': True, 'user': authenticated_user.id, 'storage': authenticated_user.storage.id, 'admin' : authenticated_user.is_superuser}, status=201)
        else:
          print(111)
          return Response(json.dumps({"errors": {"username": ["Invalid credentials"]}}), status=403)
      else:
