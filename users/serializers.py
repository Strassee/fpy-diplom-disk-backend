...
class UserRegSerializer(serializers.ModelSerializer):
  ...
  def validate_password(self, value):
    password_regexp = re.compile(r'(?=.*[0-9])(?=.*[!@#$%^&*()—_+=;:,.\/?\\|`~\[\]\{\}])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z!@#$%^&*()—_+=;:,.\/?\\|`~\[\]\{\}]{6,}')
  ...
  def validate_email(self, value):
    email_regexp = re.compile(r'^((?!\.)[\w\-_.]*[^.])(@\w+)(\.\w+(\.\w+)?[^.\W])$', flags=re.IGNORECASE)
...
class UserUpdateByAdminSerializer(serializers.ModelSerializer):
  ...
  def validate(self, attrs):
    ...
    if self.instance.id == 1 and (attrs['is_superuser'] == False or attrs['is_staff'] == False or attrs['is_active'] == False):
...
class UserUpdateByUserSerializer(serializers.ModelSerializer):
  class Meta:
    ...
    read_only_fields = ['id', 'username', 'email', 'last_login', 'date_joined', 'is_superuser', 'is_staff', 'is_active', 'groups', 'user_permissions']
